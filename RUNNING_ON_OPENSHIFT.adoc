...
=== Deploying
All quickstarts will have a yaml file, known as a Helm chart, to deploy the application with Helm. Generally this will be in `charts/helm.yaml` under each quickstart's directory. In some cases quickstarts will contain more than one to cover different cases. In this case the quickstart will point out the differences in the configuration between the files.
...
deploy:
  replicas: 4
----
We will explain the contents of this in more detail in the <<_deploying>> section. For now, the shown Helm chart contains two top level sections:
