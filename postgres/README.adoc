...
=== Adding our secret
link:./openshift-secret.yaml[openshift-secret.yaml] contains a secret called `postgres-db-secret` with base64 encoded values of the most sensitive data needed to connect to our database. See link:../RUNNING_ON_OPENSHIFT.adoc#_adding_a_secret_to_openshift[Adding a secret to OpenShift] for more information.
